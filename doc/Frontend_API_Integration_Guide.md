...
### 3.5. Chi tiết sản phẩm + Thống kê Rating / Lượt mua

#### 3.5.1. Lấy chi tiết sản phẩm (kèm tổng số đã bán)
- **Endpoint**: `GET /api/products/{id}/detail`
- **Mô tả**: Trả về thông tin chi tiết 1 sản phẩm để vẽ trang Product Detail, bao gồm cả số lượng đã bán (`totalSold`).
- **Yêu cầu Header**: Không bắt buộc Token (public GET).
- **Response (ví dụ)**:
```json
{
  "id": 10,
  "name": "Giày thể thao Nike Air Zoom",
  "price": 1500000,
  "description": "Giày chạy bộ chuyên nghiệp...",
  "stock": 20,
  "imageUrl": "https://domain.com/uploads/nike-air-zoom.jpg",
  "brand": "Nike",
  "salePrice": 1200000,
  "sku": "NIKE-AIR-001",
  "status": "ACTIVE",
  "variations": "M:Đen,42;L:Trắng,43",
  "averageRating": 0.0,
  "totalSold": 35
}
```
- **Gợi ý sử dụng ở Frontend (React ProductDetail.jsx)**:
  1. Ở `useEffect`, gọi `GET /api/products/{id}/detail` để lấy dữ liệu sản phẩm.
  2. Dùng `price`, `stock`, `imageUrl`, `totalSold` để hiển thị giống Shopee (giá + số lượt mua).
  3. Trường `variations` có thể parse để hiển thị size/màu nếu cần ở vòng sau.

#### 3.5.2. Lấy danh sách đánh giá (Review) cho sản phẩm
- **Endpoint**: `GET /api/reviews/product/{productId}`
- **Mô tả**: Trả về danh sách các đánh giá của khách đã mua hàng.
- **Yêu cầu Header**: Public (không cần Token).
- **Response (ví dụ)**:
```json
[
  {
  ...
  },
  {
    "id": 2,
    "rating": 4,
    "comment": "Đóng gói ổn, sẽ ủng hộ tiếp.",
...
```
- **Hướng dẫn vẽ UI**:
  - Map qua mảng review để hiển thị:
    - Tên (có thể ẩn bớt email, chỉ hiển thị phần trước @ hoặc che bớt).
    - Rating: vẽ số sao (1–5) dựa vào `rating`.
    - Ngày đánh giá: format từ `createdAt` sang định dạng `dd/MM/yyyy`.

#### 3.5.3. Thống kê rating & lượt mua (dùng cho header của trang Product Detail)
- **Endpoint**: `GET /api/reviews/product/{productId}/stats`
- **Mô tả**: Lấy sao trung bình và tổng số sản phẩm đã bán.
- **Response (ví dụ)**:
```json
{
  "averageRating": 4.8,
...
```
- **Sử dụng ở Frontend**:
  - Gọi API này song song với `/api/products/{id}/detail`.
  - Dùng `averageRating` để hiển thị số sao trung bình (VD: 4.8/5) và số lượng đánh giá.
  - Dùng `totalSold` để hiển thị text như: `Đã bán 120`.

#### 3.5.4. Gửi đánh giá sản phẩm (sau khi mua)
- **Endpoint**: `POST /api/reviews`
- **Mô tả**: Cho phép user đã mua sản phẩm gửi đánh giá (Rating + Comment). Backend sẽ tự kiểm tra trong `OrderItem` xem user đã từng mua sản phẩm đó chưa.
- **Yêu cầu Header**:
...
```json
{
  "productId": 10,
  "rating": 5,
  "comment": "Giày đi rất êm, chuẩn size, đóng gói kỹ."
}
```
- **Response (Thành công)**: Trả về object Review vừa tạo.
- **Luồng UI gợi ý**:
  1. Ở trang `Orders` hoặc `ProductDetail`, nếu user đã mua sản phẩm → hiển thị nút **"Viết đánh giá"**.
  2. Khi bấm, mở Modal chứa form (Rating 1–5 + Textarea Comment).
  3. Submit form → gọi `POST /api/reviews` với token.
  4. Nếu 200 OK → đóng Modal, gọi lại `GET /api/reviews/product/{id}` để refresh danh sách.
